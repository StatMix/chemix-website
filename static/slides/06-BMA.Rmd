...
When γ = 0: No model averaging takes place. In this case, only the "best" model (the one with the highest posterior probability) is used for estimation or prediction.
When γ = 1: Complete model averaging is performed. All models in the model set are given equal weight, and their predictions or parameter estimates are combined.
...
```
> what does the sticks here represent? It will be confusing for people who first see these plots! Check 4.1 of that application paper.
...
> You can comment on: xx does not have significant effect on BMI (CI include 0). xx significant positive effect. .. negative effect. etc. 

> explain the options for estimator
...
> `predict` provides model fitting and prediction. explain what does each output represent: fit, Ybma, Ypred, xxx

> `fitted` produce the fitted values, which is equvalent to predict()$fit
...
> Can you organize this section as what you did in Prior Selection section? List all possible options, and explain each option. Then provide recommendation and comparison (pros and cons) for each choice. Can you use ggplot2 to visualize CI from all the model together so to provide a clear comparison.
...
> Can you find provide some recommendations for different priors under different situations? Can you find some resources talking about pros and cons for different priors? The description you provided is very clear to statiscians, but may hard for people who have little knowledge on Bayesian statistics. They need a guidance 
...
> Probably we can summarise these point and write a conclusion similar to what Bradley has in WQS tutorial.
