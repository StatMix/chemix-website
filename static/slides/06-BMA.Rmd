...
nhanes_URX<-nhanes1518%>%
  select(URXUCR, URXCNP,URXCOP,URXECP,URXHIBP,URXMBP,URXMC1,URXMEP,URXMHBP,URXMHH)%>%na.omit()
```

We fit a BMA model on the dataset:

```{r}
x.nhanes <-log(nhanes_URX[-8])
y.nhanes <-log(nhanes_URX[8])
...
We then explore the BMI value as a categorical between 1 (denoting high BMI value) and 0 (denoting low BMI value), with BMI value of 22 as the threshold:

```{r}
BMI_category <- nhanes_na_removed %>%
...
```{r}
nhanes.bic.glm<-bic.glm(BMI_Level~income+WTINT2YR+WTMEC2YR, data=BMI_category, glm.family="binomial")
summary(nhanes.bic.glm)
```
