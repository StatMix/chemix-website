...
Weighted Quantile Sum Regression (WQS) allows researchers to understand the effects of highly correlated components. In a previous tutorial, we used the `gWQS` package to implement modeling fitting, inference, and diagnostics. An additional method used to employ Weighted Quantile Sum Regression is the `qgcomp` package. `qgcomp` package utilizes g-computation, a causal effect estimating method. This method relaxes the **directional homogeneity assumption** of WQS, here the components do not have to contribute in the same direction. Compared to WQS, g-computation can provide unbiased estimation and appropriate confidence interval coverage. Additionally, `qgcomp` has greater computation efficiency and it allows more flexibility on modeling non-linear relationship.

In this tutorial, we will demonstrate how the `qgcomp` can provide additional flexibility when analyzing chemical mixtures. Specifically, we will explore applications for (1) linear model (2) linear model with extra covariates (3) non-linear model (4) missing data, limits of detection, and multiple imputation
...
$$\mathbb{E}(Y | \mathbf{Z,\psi,\eta}) = g(\psi_0 + \psi_1 S_q + \mathbf{\eta Z})$$ In the model above, $g(\cdot)$ is a link function, $\psi_0$ represents the intercept while $S_q$ is the combined effect of the exposures (predictor variables)$S_q =\sum_{j=1}^d w_j{\mathbf{X}_{q,j}}$. $\mathbf{\eta Z$ is the error term, demonstrating the uncertainty in the model. Lastly, $\mathbf{X}^q$ is the transformation of of the X variable that has been sorted into quantiles.
...
`degree` - the number of parameters estimated in the model fitting process
...
As demonstrated above, the standard linear output is very similar to that of the WQS model. Although the coefficients vary slightly, both methods produce a similar result.
...
As the quantile sum increases by 1, an individual's BMI being over vs. under the national average decreases by approximately 1.26% (exp(-0.013)-1).

When the quantile sum is approximately zero, the probability of an individual being over vs. under the national average is approximately 2.16 (exp(0.769)).
...
qcboot.fit5 <- qgcomp(BMXBMIlog~ . + .^2,
                         ...
                         nhanes[,c(chem_names_new, 'BMXBMIlog')], family=gaussian(), q=4, B=5, seed=125)
qcboot.fit5
plot(qcboot.fit5)
```
...
Notice here $\sum w_jS_{q,j} = 1, \quad \sum w_jS_{q,j}^2 = 1$. The MSM fit linear present a quadratic trend. The interpretation is similar to that of generalized linear model with quadratic terms. With one unit increase of wqs, we expect log(BMI) increase by 0.03 (=0.065-0.035).

```{r}
qcboot.fit5d <- qgcomp(BMXBMIlog~ . + .^2,
                         expnms=chem_names_new,
...
In the previous example, we specify the non-linear regression model based on our prior (quadratic terms). Here `qgcomp` can also allow learning of nonlinear trend by using splines.


Often with chemical mixtures, variables that have high correlations may have a non-linear trend (possibly quadratic). Exposures that are highly correlated with each other may have an underlying similarities, "a common environmental source or set of behaviors" (CRAN). To account for this issue, we can use the splines model which specializes in modeling quadratic terms. First, a correlation matrix can be used to determine which chemicals found in urine samples have a strong correlation. 
...
corrplot(cor_hanes, order = 'hclust')
```

From the above plot, we can see that several variables are highly correlated with each other. For this example, we will use the URXMIB and URXHIBP variables.

To find the best fit for the data, three separate models will be used: a quadratic regression model, a non-linear model, and an interaction model. To compute the models, a new package called Splines will be used. Since we will be using non-linear models, Splines uses a "piecewise" polynomial regression method to for estimation (as shown below):
...
```{r, cache=TRUE}
correlated_chems <- c('URXMIB', 'URXHIBP')
...
qc.fit6lin <- qgcomp.glm.boot(BMXBMIlog ~ URXMHNC + URXMCOH  + URXMOH + URXMHP + URXMHBP + URXMHP + URXMHH + URXMHH + URXECP + URXHIBP + URXMIB,
                         ...
                         nhanes, family=gaussian(), q=8, B=5)

qc.fit6nonlin <- qgcomp.glm.boot(BMXBMIlog ~ bs(URXMIB) + bs(URXHIBP) + URXMCOH + URXMOH+ URXMHP + URXMHBP + URXMHP + URXMHH + URXECP,
                         expnms=correlated_chems,
                         nhanes, family=gaussian(), q=8, B=5)
summary(qc.fit6lin$fit)$coefficients
summary(qc.fit6nonlin$fit)$coefficients
summary(qc.fit6lin)
plot(qc.fit6nonlin$fit)
plot(qc.fit6lin$fit)
AIC(qc.fit6lin)
AIC(qc.fit6nonlin)
```
Prior to interpreting the data, it is important to examine the residual plots. For both models, the residuals appear to generally follow the QQ plot and appear to be evenly scattered in the Residuals vs Fitted visualization. When examining the splines model, it is important to note that variables of interest with high correlations appear more than once in the regression model, this signifies???. The linear model is similar to the examples above and carries the same assumptions. 

For our model comparison, we can once again use AIC. The non-linear model using spines produces the lower AIC, -309.7716 compared to -298.9247 for the standard linear form. This means that the splines method produced a more appropriate model for the highly correlated urine samples. 
...
plot(qc.fit6lin)
plot(qc.fit6lin, suppressprint = TRUE, pointwiseref = 4) + 
  ggtitle("Linear fit")
```
Further, the model evaluation through the visualization above demonstrates the linear trend produced???
```{r}
qc.fit6nonlin
