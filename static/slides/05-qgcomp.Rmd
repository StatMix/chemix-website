...
`B` - number of bootstrap samples when using `qgcomp.glm.noboot()`. If we fit a non-linear model, we will employ bootstrap for simulation based inference. Usually we use the default value, which is 200. 
