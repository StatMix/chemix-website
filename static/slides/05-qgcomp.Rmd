...
In the previous example, we specify the non-linear regression model based on our priori (quadratic terms). Here `qgcomp` can also allow learning of nonlinear trend by using splines.

----- start here

Although this model provides a result, it is difficult when there are numerous chemical mixtures as predictors. However, we can narrow down the variables by examining a correlation matrix. It is possible that exposures that are highly correlated with each other may have an underly ing similarities, "a common environmental source or set of behaviors."
...
qc.fit6lin <- qgcomp.glm.boot(BMXBMIlog ~ URXMHNC + URXMCOH  + URXMOH + URXMHP + URXMHBP + URXMHP + URXMHH + URXMHH + URXECP + URXHIBP + URXMIB,
                         ...
                         nhanes, family=gaussian(), q=8, B=5,degree=2)

qc.fit6nonlin <- qgcomp.glm.boot(BMXBMIlog ~ bs(URXMHNC,df = 3) + URXMCOH + URXMOH+ URXMHP + URXMHBP + URXMHP + URXMHH + URXMHH + URXECP + URXHIBP + URXMIB,
                         expnms=correlated_chems,
