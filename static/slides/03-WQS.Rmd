...
Once the datum is loaded, we can proceed to understand how our chemical exposures (detected in urine samples) impact the body mass index of participants.

Similar to the previous tutorial where age is filtered, we only want to consider participants in the data that are above 18 years old and take the natural log of BMI. We centered the age at 18 to assure the intercept of regression model is interpretable. To finish preparing the dataset, the N/A values are removed and the resulting datum is saved under the name `nhanes`.
...
To confirm the normality of the variable BMI (In our case, BMI refers to the logarithm transformation of the original BMI), we can create a histogram to view the shape of the observations:
...
However, if we fit a normal linear regression model to these predictors, positive and negative estimates would be produced. Intuitively, the URX variables should all have positive coefficients, however, this model is not representative of this assumption. This is a demonstration of multi-collinearity, where there is a large uncertainty (variance) when calculating the coefficients. Therefore, the output of the model may produce unusual signs for the coefficient estimates.

As shown below, some of the URX coefficients are positive, while others are negative. Instead, we want to use a gWQS model which will provide a ***unidirectional***  output, similar to a standard linear regression model.
...
or it can be written as an expanded model where $w_n$ corresponds the estimated weights for each X variable and $\beta_1$ is the predicted impact of the WQS method on all of the chemical exposures.

$$
Eg(Y)= \beta_0 + \beta_1(w_1X_1 + w_2X_2 + ... w_nX_n)
$$

Each outcome variable is assigned a "score", meaning they are placed in bins using quantiles Xi = {0, 1, 2, 3, etc.} For each exposure, there are cutoff points (or weights) that determine where each variable is placed. The weighted components are then summed and are fitted to the regression model. gWQS visualizes the assigned weights and regression fit as demonstrated in the package below:
...
If we were to repeat the sampling process many times and compute the confidence interval each time, we would expect the true wqs coefficient falls within the interval `r round(confint(results)[2,1],3)` and `r round(confint(results)[2,2],3)` in 95% of the cases. 


We may also be interested in testing whether URX variables have significant impact on BMI. Included in the regression output is the results significance tests of the coefficients. To test the significance of the model, we can use a t-distribution and t-statistic. The t-statistic for WQS is 5.214 and the corresponding p-value is extremely small (2.03e-07). Therefore, there is a significant relationship between the weighted quantiles and the predicted outcome, logBMI.
...
The first visualization produced is a bar plot showing the weights assigned to each of the URX variables. The variables with greatest weights are Mono-2-ethyl-5-carboxypentyl phthalate (URXECP), Mono-(2-ethyl-5-oxohexyl) phthalate (URXMOH), and Mono-isobutyl pthalate (URXMIB) while the remaining chemicals are not great predictors of BMI. Also shown in the visualization is a red line which shows the cutoff values - calculated by the inverse of the number of predictor variables.
...
-   AIC - This is a statistic help to evaluate the model performance, taking account of model fitting and complexity. The smaller the AIC, the better the model. 

For example, we can demonstrate AIC by comparing the current WQS model with a new WQS model (adding a covariate `age`). The AIC from the original model was -164.58 while the new model with age as a covariate has an AIC of -183.82 (see the following section). Therefore, we can say that the second model with age added is a better model, more effectively fitting the data. 

## WQS Model with Covariates

Covariates in the model are variables that we are not necessarily interested in interpreting, but may be influential to the overall model.  

In order to add a covariate using the gWQS package, the variable must be added to the regression equation, shown below. This ensures that age is not treated as a mixture, but a separate variable.
...
Notice the coefficient of age is significantly non-zero, suggesting age has significant potential effect on log(BMI). If we use AIC metric for model comparison, the AIC from the previous model was -164.58 while the new model with age as a covariate has an AIC of -183.82. Therefore, we can say that the second model with age added is a better model, more effectively fitting the data. 
