...
In order to investigate relationships between chemical exposures using the `gWQS` package, there are several packages in R that must be loaded. Similar to previous tutorials where packages were introduced, we will be using `tidyverse` and `tidymodels` for data wrangling and model building, but we will also use `ggcorrplot` for data visualization.
...
Quantiles categorize each variable where $q_i = {0, 1, 2, 3, ...}$, which are given a corresponding weight. The weighted components are then summed and are fitted to the regression model. `gWQS` visualizes the assigned weights and regression fit as demonstrated in the package below:
...
Here we present an example to xxx. 

(Add an example illustrating how to include extra predictors, such as age; How to compare these two models using AIC. Try `lrtest` in `lmtest` r pkg to conduct model comparison)
