...
For categorical variable, we need to mutate them as factors stored in the data frame first. The income category coded as the first factor level will be the baseline. When modeling categorical variables, it is common to omit one level of the categorical variable, which is known as the reference level. This is done to avoid the problem of perfect multicollinearity between the dummy variables that represent the levels of the categorical variable. Specifically, In a regression model, a categorical variable with k levels can be represented by k-1 dummy variables, with each dummy variable representing one of the levels of the categorical variable, and the omitted reference level represented by a constant term in the regression equation. By omitting one level of the categorical variable, we ensure that the sum of the k-1 dummy variables is equal to one for each observation, which eliminates the problem of perfect multicollinearity. This also allows us to interpret the coefficients of the dummy variables as the difference between the level of interest and the omitted reference level, holding all other variables in the model constant.
...
*Model Interpretation*:
-   Intercept: The intercept `round(coef(lm_category)[1],3)` means that for people in the baseline income category (income category 1 corresponding to a household income of 0 to 4,999 dollars), the BMI is expected to be `round(coef(lm_category)[1],3)` on average.

-   `income6`: The coefficient for the predictor `income6` is `round(coef(lm_category)["income6"],3)`, which means that for participants with household income category 6 (25,000 to 34,999 dollars per ear), the BMI is expected to be `round(coef(lm_category)["income6"],3)` higher than that of participants with household income in category 1 (0 to 4,999 dollars), on average.
...
For the interaction term to be interpretable, we mean center the term `income` involved in the interaction. In R, mean centering variables involves subtracting the mean value of each variable from all the observations in that variable. This can be useful in various statistical analyses, such as regression analysis, where mean centering can improve the interpretation and accuracy of the results.

To interpret the categorical variables, we use (n-1) variables to encode, and we noticed that category 1 does not appear in the model thus is the baseline.

```{r}
nhanes_income$RIDAGEYR_centered <- scale(nhanes_income$RIDAGEYR, center = TRUE, scale = FALSE)

lm(BMXBMI ~ income + RIDAGEYR_centered + income:RIDAGEYR_centered, data = nhanes_income)%>%
  summary()
...
Take income category 2 as an example, at the mean value of year 
`r round(mean(nhanes_income$RIDAGEYR),3)`, the BMI of an individual in income category 2 is expected to be 0.72331 higher than the BMI of an individual in income category 1 (baseline category).
