...
fitkm_corr <- kmbayes(y = y, Z = Z, X = X, iter = 2000, varsel = TRUE, verbose = FALSE)
...
```{r}
sel<-seq(0,1000,by=1)
TracePlot(fit = fitkm_corr, par = "beta", sel=sel)
```

```{r cache=TRUE}
mixture<-as.matrix(Z)
pred.resp.univar <- PredictorResponseUnivar(fit=fitkm_corr, sel=sel, 
                                            method="approx")

pred.resp.bivar  <- PredictorResponseBivar(fit=fitkm_corr,  min.plot.dist = 1, 
...
risks.overall <- OverallRiskSummaries(fit=pred.resp.bivar, qs=seq(0.25, 0.75, by=0.05), 
                                      q.fixed = 0.5, method = "approx",
                                      sel=sel)

risks.singvar <- SingVarRiskSummaries(fit=pred.resp.bivar, qs.diff = c(0.25, 0.75),
                                      q.fixed = c(0.25, 0.50, 0.75), 
                                      method = "approx")

risks.int <- SingVarIntSummaries(fit=pred.resp.bivar, qs.diff = c(0.25, 0.75),
