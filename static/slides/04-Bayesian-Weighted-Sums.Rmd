...
Unlike $\beta$, the specification for weights $w$ is different since it poses a constraint that should sum up to 1, $w_1 + w_2 + … + w_k = 1$. Since the posterior has the same support as that of the prior, such constraint on weights is easily addressed in Bayesian methods by specifying a prior on weights subject to the constraint. The distribution commonly used for weights is the Dirichlet distribution, denoted as $w \sim Dir(\alpha)$. $w$ represents weights with length $k$, and $\alpha$ represents concentration parameters with length $k$.
...
Let's refine this. What are the advantages of BWQS as implemented compared to WQS as implemented?  Certainly uncertainty quantification is one of them.
