...
In this model, beta 1 is the combined effect of the three mixture exposures, while w is each individual weight. In this model, weights (w1,w2,...,wn) to must sum to 1. It is important to note that the parameters in the BWS model can have informative priors assigned. However, for this example, we do not have valuable prior information, so will use a the standard normal distribution for each parameter.
...
Additional information provided by the model includes the standard error, the effective sample size (number of cases in the population), and R-hat which is the corresponds to the convergence of MCMC simulations. A reasonable effective sample size (R-hat) is typically considered reasonable if it is less that 1.01.
