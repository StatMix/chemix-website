...
A statistician can also have no prior. Suppose, unrealistically, that a statistician has never heard of rain, and therefore has no knowledge on how likely it is to occur. When that statistician is asked for the probability that it will rain tomorrow, they have no prior knowledge to go off of. Thus, they will have to use data (examining whether or not in rains for the next number of days) to form some beliefs. 
...
For example, we can use the Bayes' rule with our `nhanes` dataset to estimate the proportion of subjects in the dataset with a concentration of Mono(carboxynonyl) phthalate above 10 ng/mL that are female. For simplicity, we will only consider subjects with data points present for both the `RIAGENDR` variable, which deals with gender.

```{r filter-out-nulls}
bayesrule_ex <- nhanes1518 %>%
  filter(!is.na(RIAGENDR) & URXCNP > 10)
```

In this example, $\theta$ is the parameter we are interested in (the proportion of subjects in this subsetted dataset that are female). $y$ is our data, or `bayesrule_ex`. Let's assume we have no prior knowledge on whether men or women are more likely to have high Mono(carboxynonyl) phthalate concentrations. Thus, we would set our prior at 0.5 (we believe that it is equally likely for men and women to have concentrations above 10 ng/mL). In a complete Bayesian analysis, we would then combine this with our data (in which the proportion is 0.467) to obtain a posterior. It is important to note that the posterior we will obtain will not be the same as, for example, the posterior we would obtain if we chose a prior of 0.9. People with different priors will have different posteriors.

Note that this is an extremely simplified example. Here, we use a point estimate for our prior (0.5), but usually our prior comes from a probability distribution. For example, again assuming we have no prior knowledge, one prior we could choose is a $Uniform(0, 1)$ distribution. This stipulates that we believe the true proportion has an equal chance of being any value between 0 and 1. 

### Posteriors

Just like how our priors usually come from probability distributions, so do our posteriors. This is a key distinction from frequentist statistics, where $\theta$ is a fixed value. In Bayesian statistics, $\theta$ is a **random variable**, or a quantity that can take on different values based on different outcomes of an experiment. For example, if our posterior for $\theta$ is a $Uniform(0, 1)$ distribution, then $\theta$ will take on an array of different values from 0 to 1 if we repeatedly sampled from this distribution. The fixed-value estimate for $\theta$ in frequentist statistics usually maps to the mean of this posterior distribution in Bayesian statistics (which, in the case of a $Uniform(0, 1)$ distribution is 0.5). 

When conducting hypothesis tests or linear regressions using the frequentist approach, we often find it useful to create a **confidence interval** for $\theta$. Confidence intervals help us estimate the (un)certainty of a parameter with a lower and upper bound. For example, let's say we create a 95% confidence interval for $\theta =$ the mean age in our datase; we find the lower bound to be 32 and the upper bound to be 47. We could then interpret this confidence interval as "We are 95% confident that the true mean age is between 32 and 47 years old." If we constructed many more similar confidence intervals, we would expect 95% of them to contain the true population mean.

The Bayesian equivalent to a confidence interval is known as a **credible interval**, which has a simpler interpretation. For example, a 95% credible interval is between any lower bound $L$ and any upper bound $U$ such that the posterior probability of $L < p < U = 0.95$. We use the quantiles of the posterior distribution for $\theta$ to obtain $L$ and $U$. In the case of a 95% interval, these are the 0.025 and 0.975 quantiles (the middle 95% of the data). If we find the 95% credible interval for the mean age in our dataset to have a lower bound of 32 and an upper bound of 47, then we can simply say that "There is a 95% probability that the true mean age is between 32 and 47 years old."
...
$$\hat{BMI} = 29.53 - 0.07(\hat{Age\_sc}) + 0.28(\hat{Waist\_sc}) - 3.49(\hat{Height\_sc}) + 7.70(\hat{Weight\_sc})$$
...
There are many different types of priors we can choose from, and they prove handy in different cases. This section will discuss common priors used for coefficients ($\beta$) and variances ($\sigma^2$).
...
We get the model $\hat{BMI} = 36.19 - 0.12(\hat{Age})$. We will compare this to models using specified priors.
