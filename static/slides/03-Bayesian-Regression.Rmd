---
title: "Tutorial 03 - Bayesian Regression"
...
ggplot(nhanes1518, aes(x = BMXBMI)) + 
  ...
  labs(title = 'Distribution of BMI in Dataset')
```
...
```{r image4, echo=FALSE}
knitr::include_graphics("img/bayesian-regression/uniform.png")
```

For our next model, we will assign all of our coefficients to a **uniform distribution**. In this kind of distribution, we set a range of values (for example, 0 to 1, or a to b as illustrated above) in which every outcome is equally likely. 
...
for the inverse gamma leave the mean in same place and increase variance


NEXT STEPS OUTLINE 

we want to see if bayesian worked: look at trace plots
MCMC diagnostics
look at effect of sample size
look at model diagnostics, fit criteria, WAIC diagnostics 
look at AIC for model comparison in choosing different priors

AIC is just based on likelihood but doesn't include anything about the prior